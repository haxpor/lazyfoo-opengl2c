...
gl_LTexture* checkerboard_texture = NULL;
...
bool usercode_loadmedia()
{
  ...
  for (int i=0; i<CHECKERBOARD_PIXEL_COUNT; i++)
  {
    ...
    // concept: first half is to set & toggle bit on individual row
    // second half is to set bit on each column
    // 16 is the size of each checker square
  ...
  // load texture
  checkerboard_texture = gl_LTexture_new();
  if (!gl_LTexture_load_texture_from_pixels32(checkerboard_texture, checkerboard, CHECKERBOARD_WIDTH, CHECKERBOARD_HEIGHT))
  {
    SDL_Log("failed to generate checkerboard texture");
    return false;
  }
...
void usercode_render()
{
  ...
  // calculate centered offsets for checkerboard
  GLfloat x = (g_logical_width - checkerboard_texture->width) / 2.0f;
  GLfloat y = (g_logical_height - checkerboard_texture->height) / 2.0f;

  // render checkerboard
  gl_LTexture_render(checkerboard_texture, x, y);
...
void usercode_close()
{
  ...
  {
    gl_LTexture_free(checkerboard_texture);
  }
}
