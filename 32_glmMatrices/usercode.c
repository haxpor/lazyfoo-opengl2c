...
// basic shader
static gl_LPlainPolygonProgram2D* basic_shader = NULL;
...
{
	// bind shader
	gl_LShaderProgram_bind(basic_shader->program);

	// copy calculated projection matrix to basic shader's then update to shader
	glm_mat4_copy(g_projection_matrix, basic_shader->projection_matrix);
	gl_LPlainPolygonProgram2D_update_projection_matrix(basic_shader);

	// copy calculated modelview matrix to basic shader's
	glm_mat4_copy(g_base_modelview_matrix, basic_shader->modelview_matrix);

	// unbind
	gl_LShaderProgram_unbind(basic_shader->program);
}
...
bool usercode_loadmedia()
{
  // TODO: Load media here...
	...
	basic_shader = gl_LPlainPolygonProgram2D_new(raw_shader_program);
	if (!gl_LPlainPolygonProgram2D_load_program(basic_shader))
	{
		return false;
	}

	// bind and set it to be the active shader
	if (!gl_LShaderProgram_bind(basic_shader->program))
	{
		return false;
	}

	// copy calculated projection matrix to basic shader's
	glm_mat4_copy(g_projection_matrix, basic_shader->projection_matrix);
	gl_LPlainPolygonProgram2D_update_projection_matrix(basic_shader);

	// copy calculated modelview matrix to basic shader's
	glm_mat4_copy(g_base_modelview_matrix, basic_shader->modelview_matrix);
	gl_LPlainPolygonProgram2D_update_modelview_matrix(basic_shader);
...
void usercode_render()
{
  ...
  // TODO: render code goes here...
	...
	// manipulate matrix to center at the screen
	glm_mat4_copy(g_base_modelview_matrix, basic_shader->modelview_matrix);
	glm_translate(basic_shader->modelview_matrix, (vec3){g_logical_width / 2.f, g_logical_height / 2.f, 0.f});
	gl_LPlainPolygonProgram2D_update_modelview_matrix(basic_shader);
	gl_LPlainPolygonProgram2D_set_color(basic_shader, 0.f, 1.f, 1.f, 1.f);
...
void usercode_close()
{
	if (basic_shader != NULL)
		gl_LPlainPolygonProgram2D_free(basic_shader);
