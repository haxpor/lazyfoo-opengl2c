...
{
  shader_program = gl_LPlainPolygonProgram2D_new();
  ...
  // set to use such loaded shader program
  if (!gl_LShaderProgram_bind(shader_program))
  {
    return false;
  }
  return true;
...
void usercode_render()
{
  ...
  // render cyan quad in the center
