...
// -- section of variables for maintaining aspect ratio -- //
gl_LTexture* circle_texture = NULL;
...
bool usercode_loadmedia()
{
  ...
  if (!gl_LTexture_load_texture_from_file(circle_texture, "circle.png"))
  {
    SDL_Log("Unable to load texture");
    return false;
  }

  // lock texture preparing to manipulate data
  if (!gl_LTexture_lock(circle_texture))
  {
    SDL_Log("Unable to lock texture");
    return false;
  }

  // we want to replace cyan color
  // note: texture is in .png RGB format, so it's fully opaque
  const GLuint target_color = 0xFF00FFFF;
  // calaculate pixel count based on real texture's width / height after padded
  const GLuint pixel_count = circle_texture->physical_width_ * circle_texture->physical_height_;
  for (int i=0; i<pixel_count; i++)
  {
    if (circle_texture->pixels[i] == target_color)
    {
      // replace with fully transparent black color
      // we store byte data in little enddian so ARGB (we store opaque black color)
      circle_texture->pixels[i] = (0xff << 24) | (0 << 16) | (0 << 8) | 0;
    }
  }

  // make diagonal lines
  for (int y = 0; y < circle_texture->physical_height_; y++)
  {
    for (int x = 0; x < circle_texture->physical_width_; x++)
    {
      if (y % 10 != x % 10)
      {
        gl_LTexture_set_pixel32(circle_texture, x, y, 0xff000000);
      }
    }
  }

  // unlock texture
  if (!gl_LTexture_unlock(circle_texture))
  {
    SDL_Log("Unable to unlock texture");
    return false;
  }
  return true;
}
...
void usercode_render()
{
  ...
  gl_LTexture_render(circle_texture, (g_logical_width-circle_texture->width)/2.0f, (g_logical_height-circle_texture->height)/2.0f, NULL);
...
void usercode_close()
{
  if (circle_texture != NULL)
    gl_LTexture_free(circle_texture);
}
