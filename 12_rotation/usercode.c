...
// -- section of variables for maintaining aspect ratio -- //
static gl_LTexture* arrow_texture = NULL;
...
bool usercode_loadmedia()
{
  ...
  arrow_texture = gl_LTexture_new();
  if (!gl_LTexture_load_texture_from_file(arrow_texture, "arrow.png"))
  {
...
void usercode_render()
{
  ...
  gl_LTexture_render(arrow_texture, (g_logical_width - arrow_texture->width)/2.f, (g_logical_height - arrow_texture->height)/2.f, NULL, NULL, rot_angle);
...
void usercode_close()
{
  if (arrow_texture != NULL)
    gl_LTexture_free(arrow_texture);
}
