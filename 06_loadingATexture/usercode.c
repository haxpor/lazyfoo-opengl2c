...
gl_LTexture* opengl_texture = NULL;
...
bool usercode_loadmedia()
{
  ...
  if (!gl_LTexture_load_texture_from_file(opengl_texture, "texture.png"))
  {
    SDL_Log("failed to load opengl texture");
    return false;
  }
  return true;
}
...
void usercode_render()
{
  ...
  gl_LTexture_render(opengl_texture, 0.f, 0.f);
