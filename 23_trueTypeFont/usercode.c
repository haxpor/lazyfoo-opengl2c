...
// -- section of variables for maintaining aspect ratio -- //
gl_LFont* ttf_font = NULL;
gl_LFont* ttf_font2 = NULL;
gl_LFont* ttf_font3 = NULL;
...
bool usercode_loadmedia()
{
  ...
  gl_LTexture* raw_texture = gl_LTexture_new();
  gl_LSpritesheet* spritesheet = gl_LSpritesheet_new(raw_texture);
  ttf_font2 = gl_LFont_new(spritesheet);

  if (!gl_LFont_load_freetype(ttf_font2, "Minecraft.ttf", 40))
  {
    SDL_Log("Unable to load ttf font 2");
    return false;
  }
  }
  ...
  gl_LTexture* raw_texture = gl_LTexture_new();
  gl_LSpritesheet* spritesheet = gl_LSpritesheet_new(raw_texture);
  ttf_font3 = gl_LFont_new(spritesheet);

  if (!gl_LFont_load_freetype(ttf_font3, "8bitwonder.ttf", 30))
  {
    SDL_Log("Unable to load ttf font 3");
    return false;
  }
  }
  return true;
...
void usercode_render()
{
  ...
  gl_LFont_render_text(ttf_font, "Now rendering with TTF!", 5.f, 0.f + 5.f);
  gl_LFont_render_text(ttf_font2, "Now rendering with TTF!", 5.f, g_logical_height / 2.0f);
  gl_LFont_render_text(ttf_font3, "Now rendering with TTF!", 5.f, g_logical_height - 30.f - 5.f);
...
{
  if (ttf_font != NULL)
    // this will take care of its underlying companions of gl_LTexture and gl_LSpritesheet as well
    gl_LFont_free(ttf_font);
  if (ttf_font2 != NULL)
    gl_LFont_free(ttf_font2);
  if (ttf_font3 != NULL)
    gl_LFont_free(ttf_font3);
}
