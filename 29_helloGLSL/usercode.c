...
// -- section of variables for maintaining aspect ratio -- //
static gl_LPlainPolygonProgram2D* pp_program = NULL;
...
bool usercode_load_graphics_program()
{
  // TODO: Load graphics programs here ie. shaders
  pp_program = gl_LPlainPolygonProgram2D_new();
  if (!gl_LPlainPolygonProgram2D_load_program(pp_program))
  {
    return false;
  }
  // bind (set to use this shader as active)
  gl_LShaderProgram_bind(pp_program);
  return true;
}
...
void usercode_render()
{
  ...
  // render solid quad at the center
...
{
  if (pp_program != NULL)
    gl_LPlainPolygonProgram2D_free(pp_program);
}
