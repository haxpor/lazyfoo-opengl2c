...
bool init() {
  ...
  gWindow = LWindow_new("29 - Hello GLSL", SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_OPENGL, 0);
  if (gWindow == NULL) {
...
bool setup()
{
  ...
  if (!usercode_load_graphics_program())
  {
    SDL_Log("Failed to load graphics programs");
    return false;
  }
