...
gl_LTexture* dds_dxt5_nomipmaps_texture = NULL;
...
bool usercode_loadmedia()
{
  ...
  // dxt5 - no mipmaps
  dds_dxt5_nomipmaps_texture = gl_LTexture_new();
  if (!gl_LTexture_load_dds_texture_from_file(dds_dxt5_nomipmaps_texture, "zombie-512-dxt5-nomipmap.dds"))
  {
    SDL_Log("Unable to load dds (no mipmaps) texture");
    return false;
  }
...
void usercode_render()
{
  ...
  gl_LTexture_render(dds_dxt5_texture, g_logical_width/4.0f * 3 - dds_dxt5_texture->width/2.0f, g_logical_height/2.0f - dds_dxt5_texture->height/2.0f, NULL);
  gl_LTexture_render(dds_dxt5_nomipmaps_texture, g_logical_width/4.0f - dds_dxt5_nomipmaps_texture->width/2, g_logical_height/2.0f - dds_dxt5_nomipmaps_texture->height/2.0f, NULL);
...
void usercode_close()
{
  ...
  if (dds_dxt5_nomipmaps_texture != NULL)
    gl_LTexture_free(dds_dxt5_nomipmaps_texture);
}
