...
void usercode_handle_event(SDL_Event *e, float delta_time)
{
  if (e->type == SDL_KEYDOWN)
  {
    ...
    {
      // cycle through texture repeatitions
      texture_wrap_type++;
      if (texture_wrap_type >= 2)
      {
        texture_wrap_type = 0;
      }

      // set texture repeatition
      ...
      if (texture_wrap_type == 0)
      {
        ...
        SDL_Log("Use GL_REPEAT");
      }
      else
      {
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
        SDL_Log("Use GL_CLAMP");
      }
    }
  }
}
...
void usercode_render()
{
  ...
  // this would calculate how many repeatitive texture should be map to fully render on screen's dimension
  GLfloat texture_right = (GLfloat)g_logical_width / (GLfloat)repeating_texture->width;
  GLfloat texture_bottom = (GLfloat)g_logical_height / (GLfloat)repeating_texture->height;
