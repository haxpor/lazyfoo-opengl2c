...
// -- section of variables for maintaining aspect ratio -- //
static gl_LTexture* stretched_texture = NULL;
static LFSize stretch_size;
static GLenum filtering = GL_LINEAR;
...
bool usercode_init(int screen_width, int screen_height, int logical_width, int logical_height)
{
  ...
  // set stretch size
  stretch_size.w = g_logical_width;
  stretch_size.h = g_logical_height;
...
bool usercode_loadmedia()
{
  ...
  stretched_texture = gl_LTexture_new();
  if (!gl_LTexture_load_texture_from_file(stretched_texture, "mini_opengl.png"))
  {
...
void usercode_handle_event(SDL_Event *e, float delta_time)
{
  if (e->type == SDL_KEYDOWN)
  {
    ...
    // switch texture filtering option for stretched texture
    else if (k == SDLK_q)
    {
      // bind texture for modification
      glBindTexture(GL_TEXTURE_2D, stretched_texture->texture_id);
...
void usercode_render()
{
  ...
  gl_LTexture_render(stretched_texture, 0.0f, 0.0f, NULL, &stretch_size);
...
void usercode_close()
{
  if (stretched_texture != NULL)
    gl_LTexture_free(stretched_texture);
}
